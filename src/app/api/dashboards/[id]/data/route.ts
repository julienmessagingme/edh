...
export async function GET(
  ...
  //     `dashboards.is_shared`. Client raw (multi-école) → on vérifie l'école
  //     de la campagne. On évite la requête si le viewer est déjà l'auteur.
  //   - Tableau libre → son propre is_shared.
  // (Avant : `campaign_id != null` rendait tout tableau de campagne lisible
  // par n'importe quel user de l'école — fuite des compteurs d'une campagne
  // privée.)
  ...
  if (dash.campaign_id) {
    if (dashAny.created_by === user.userId) {
      ...
      visible =
        !!camp &&
        camp.school_slug === scope &&
        (camp.created_by === user.userId || camp.is_shared === true);
    }
  } else {
  ...
  async function computeRef(r: RefRow): Promise<ComputedRef> {
    if (r.step_type === "mm_event") {
      ...
      if (isPhoneCarrier) {
        ...
        try {
          const textValues = await fetchOccurrenceTextValues(refSchool, evNs);
