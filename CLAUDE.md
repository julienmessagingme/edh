...
| 37 — Stats refactor : sous-nav **par type de viz** — « Mes tableaux » → **« Pie charts »** (création forcée pie, `DashboardTypeRadio` supprimé ; les tableaux existants restent listés) + « Campagnes » → **« Funnel »** (renommage user-facing campagne→funnel, fonctionnel 100 % inchangé). URLs de route (`/dashboards`, `/campaigns`) et concept `campaign` côté DB/API/code inchangés. **Comparaison de 2 périodes** (`period-compare-dialog.tsx`) : bouton « Comparer les périodes » à côté du sélecteur de période du builder → modale 3 modes (manuel / 2 derniers mois glissants / derniers trimestres calendaires : A = trimestre en cours à date, B = précédent complet), fetch `/data?preset=custom&from&to` ×2 (déjà supporté), rendu 2 graphes côte à côte (réutilise `PieChartViz`/`FunnelChart`) + table comparative A/B/**Δ/Δ %** (vert/rouge). Sur les 2 onglets (builder partagé) | ✅ |
| 36 — Fix bug **switch d'école = données périmées** : les listes scope-dépendantes (`/api/campaigns`, `/api/dashboards`, `/api/dashboards/palette`, `/api/stats/custom-events`, `/api/stats/redirects`, + les 3 fetches de chargement du builder, la page campagne, le dialog) ont la **même URL** quelle que soit l'école (le scope vient du cookie) → le navigateur resservait la réponse cachée de l'école précédente (« les tableaux d'EFAP apparaissent quand je passe sur Brassart »). Ajout `cache: "no-store"` partout — même fix que le bug graphe/date sur `/data` (cf. `brain/LEARNINGS.md` 2026-06-26). Seul `/data` l'avait déjà | ✅ |
